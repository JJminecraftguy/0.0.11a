...
public class LevelRenderer implements LevelListener {
	...
	public void render(Player player, int layer) {
		...
		int id = this.textures.loadTexture("/terrain.png", GL11.GL_LINEAR);
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, id);
		Frustum frustum = Frustum.getFrustum();

		for(int i = 0; i < this.chunks.length; ++i) {
			if(frustum.isVisible(this.chunks[i].aabb)) {
				this.chunks[i].render(layer, player, this.textures);
			}
		}
	...
	public void renderHit(HitResult h, int mode, int tileType) {
		...
		if(mode == 0) {
			byte shape = this.level.getShape(h.x, h.y, h.z);
			t.init();

			for(int br = 0; br < 6; ++br) {
				Tile.rock.renderFaceNoTexture(t, shape, h.x, h.y, h.z, br);
			}
